...
description: "How encrypted SOPS data is decrypted for NixOS system and Home Manager secrets."
section: "Nix"
weight: 180
---
...
sops --encrypt --in-place lib/secrets/owais.yaml

# Edit the encrypted file (add your actual SSH keys)
sops lib/secrets/owais.yaml
```

```bash
SOPS_AGE_KEY_FILE=$(pwd)/age.txt sops lib/secrets/owais.yaml
```
...
**Edit secrets**: `SOPS_AGE_KEY_FILE=$(pwd)/age.txt sops lib/secrets/owais.yaml`
...
- View encrypted: `SOPS_AGE_KEY_FILE=$(pwd)/age.txt sops -d lib/secrets/owais.yaml`
...
Active NixOS hosts use the same encrypted `lib/secrets/owais.yaml` file and personal age key for unified secret management.
